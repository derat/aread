...
func (p *Processor) checkContent(pi PageInfo, content string) error {
	...
	for i, entry := range pats {
		...
		p.cfg.Logger.Printf("url=%v content=%q urlpat=%q (%v) contentpat=%q (%v)", pi.OriginalUrl, content, entry[0], urlRegexp.MatchString(pi.OriginalUrl), entry[1], contentRegexp.MatchString(content))
